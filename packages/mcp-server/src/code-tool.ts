...
const prompt = `Runs JavaScript code to interact with the Beyond Presence API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { BEY_API_KEY: requireValue(readEnv('BEY_API_KEY') ?? client.apiKey, 'set BEY_API_KEY environment variable or provide apiKey client option'), BEYOND_PRESENCE_BASE_URL: readEnv('BEYOND_PRESENCE_BASE_URL') ?? client.baseURL ?? undefined };
