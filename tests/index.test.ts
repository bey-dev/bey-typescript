...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['BEYOND_PRESENCE_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: BeyondPresence) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    const client = new BeyondPresence({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new BeyondPresence({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'BEYOND_PRESENCE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new BeyondPresence({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['BEYOND_PRESENCE_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new BeyondPresence({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['BEYOND_PRESENCE_BASE_URL'] = 'https://example.com/from_env';
      const client = new BeyondPresence({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['BEYOND_PRESENCE_BASE_URL'] = ''; // empty
      const client = new BeyondPresence({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.bey.dev/')
    });

    test('blank env variable', () => {
      process.env['BEYOND_PRESENCE_BASE_URL'] = '  '; // blank
      const client = new BeyondPresence({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.bey.dev/')
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new BeyondPresence({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['BEYOND_PRESENCE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new BeyondPresence({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new BeyondPresence({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new BeyondPresence({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new BeyondPresence({ apiKey: 'My API Key', fetch: testFetch });
